...
OpenTofu module for `proxmox dns` resources.

Migrated from `kholisrag/labirin` `modules/opentofu/proxmox/dns/0.1.0`.
...
```hcl
terraform {
  source = "git::https://github.com/kholisrag/iac-modules.git?ref=proxmox-dns/v0.1.0"
}
```
...
Floating tags `proxmox-dns/v0` and `proxmox-dns/v0.1` track the latest matching release.
