...
OpenTofu module for `proxmox time` resources.

Migrated from `kholisrag/labirin` `modules/opentofu/proxmox/time/0.1.0`.
...
```hcl
terraform {
  source = "git::https://github.com/kholisrag/iac-modules.git?ref=proxmox-time/v0.1.0"
}
```
...
Floating tags `proxmox-time/v0` and `proxmox-time/v0.1` track the latest matching release.
