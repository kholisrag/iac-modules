...
OpenTofu module for `proxmox files` resources.

Migrated from `kholisrag/labirin` `modules/opentofu/proxmox/files/0.1.0`.
...
```hcl
terraform {
  source = "git::https://github.com/kholisrag/iac-modules.git?ref=proxmox-files/v0.1.0"
}
```
...
Floating tags `proxmox-files/v0` and `proxmox-files/v0.1` track the latest matching release.
