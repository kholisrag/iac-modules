...
OpenTofu module for `proxmox hosts` resources.

Migrated from `kholisrag/labirin` `modules/opentofu/proxmox/hosts/0.1.0`.
...
```hcl
terraform {
  source = "git::https://github.com/kholisrag/iac-modules.git?ref=proxmox-hosts/v0.1.0"
}
```
...
Floating tags `proxmox-hosts/v0` and `proxmox-hosts/v0.1` track the latest matching release.
