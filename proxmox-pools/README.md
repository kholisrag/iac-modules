...
OpenTofu module for `proxmox pools` resources.

Migrated from `kholisrag/labirin` `modules/opentofu/proxmox/pools/0.1.0`.
...
```hcl
terraform {
  source = "git::https://github.com/kholisrag/iac-modules.git?ref=proxmox-pools/v0.1.0"
}
```
...
Floating tags `proxmox-pools/v0` and `proxmox-pools/v0.1` track the latest matching release.
